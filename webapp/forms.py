...
class newUserForm(FlaskForm):
    ...
    def validate_userid(self, uid):
        user = db.session.query(User.user_id).filter_by(user_id=uid.data)
        if user:
...
class newBusinessForm(FlaskForm):
    ...
    def validate_business_id(self, bid):
        business = db.session.query(Busines.business_id).filter_by(business_id=bid.data)
        if business:
...
class newReviewForm(FlaskForm):
    ...
    def validate_review_id(self, rid):
        review = db.session.query(Review.review_id).filter_by(review_id=rid.data)
        if review:
